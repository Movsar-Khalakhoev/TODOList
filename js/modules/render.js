...
const foldersContainer = document.querySelector('.left__folders');
const tasksContainer = document.querySelector('.right__tasks');

const renderFolders = function() {
	foldersContainer.innerHTML = '';
	for (let folder in tasks.folders) {
		const addedFolder = document.createElement('div');
		addedFolder.classList.add('left__folder');
		addedFolder.setAttribute('id', tasks.folders[folder].id);
		addedFolder.innerHTML = `
			<div class="left__folder-overlay" title="${tasks.folders[folder].name}"></div>
			<div class="left__folder-circle" style="background-color: ${tasks.folders[folder].circleColor}"></div>
			<div class="left__folder-text element-text">${tasks.folders[folder].name}</div>
		...
		foldersContainer.append(addedFolder);
	}
};
...
const renderTasks = function() {
	...
	if (activeFolder.tasks.length == 0) {
		tasksContainer.textContent = 'Задачи отсутствуют';
		tasksContainer.style.cssText = 'border: none; font-size: 20px; color: #767676;';
		return;
	} else {
		tasksContainer.style = '';
	}
	tasksContainer.innerHTML = '';
	activeFolder.tasks.forEach((task, index) => {
		const addedTask = document.createElement('div');
		addedTask.classList.add('right__task');
		addedTask.setAttribute('id', index);
		addedTask.innerHTML = `
			<label for="cbx-${index}" class="right__task-label">
				...
				<p class="right__task-text element-text">${activeFolder.tasks[index]}</p>
			</label>
		...
		tasksContainer.append(addedTask);

		if (activeFolder.performedTasksID.includes(index)) {
			tasksContainer.lastChild.querySelector('.right__task-hidden-input').checked = true;
		}
	});
};

function renderColors() {
	const folderFormColors = ['#a60000', '#5d8114', '#8455bf', '#1f0351', '#b69200', '#ff5600', '#f3296d', '#009641'];
...
// setActiveFolder(new Proxy(activeFolder.tasks, {
